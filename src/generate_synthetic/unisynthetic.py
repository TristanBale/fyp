...
"""
    Written by J. Wei May 2023.
    Generate univariate synthetic time series and format to .csv
"""
import math
import os
import random
...
class UniSynthetic:
    """
    Main class to generate the univariate synthetic dataset
    """
    def __init__(
            self, 
            save_path=None, 
        ...
        # we create a distribution for sine frequency to have control over class distribution
        f_support = np.random.randint(self.f_min_support, (self.f_max_support + 1), 15000)
        f_base = np.random.randint(self.f_min_base, (self.f_max_base + 1), 15000)
        ...
        self.quantile_class_sum = np.quantile(f_support, data_config["quantile_class"])
    ...
    def format_to_DataFrame(self):
        ...
        # save files
        df_signal.to_csv(os.path.join(self.save_path, 'uni_signals.csv'))
        df_cls.to_csv(os.path.join(self.save_path, 'uni_classes.csv'))
        print('Already Done!')
    ...
    def _generate_sample(self, index):
        ...
        f_support = np.random.randint(self.f_min_support, (self.f_max_support + 1), 1)[0]
            # if enum < 2:


        f_sine_sum += f_support
        f_ratio += f_support / f_base
        x_tmp, start_idx = self._generate_feature(wave="sine", f_support=f_support, f_base=f_base)
        x_sample[:, 0] = x_tmp.squeeze()
        pos_sin_wave.append(start_idx)
            # else:

        # wave = random.choice(["line", "square"])
        # x_tmp, _ = self._generate_feature(wave=wave, f_support=f_support, f_base=f_base)
        # x_sample[:, 0] = x_tmp.squeeze()

        if self.bool_snr:
            x_sample[:, 0] = self._add_noise(signal=x_sample[:, 0])
...
if __name__ == "__main__":
    save_path = "/home/TimeSeries-Interpretability-Robustness/src/generate_synthetic/UnivariateSynthetic/dataset_addnoise"
    config_path = "/home/TimeSeries-Interpretability-Robustness/configs/univariate.yaml"
    with open(config_path) as f:
        data_config = yaml.safe_load(f)
    dict_pre = UniSynthetic(save_path=save_path, data_config=data_config['properties'])
